...
from .quant.quant_modules import QuantDynaLinear, QuantLinear, QuantLinear_Act, QuantEmbedding, QuantDynaLinear_Act
...
def insert_quant_act_modules(model):
    ...
    for name, layer in model._modules.items():
        ...
        if len(layer._modules.items()) > 0:
            ...
            model._modules[name] = QuantDynaLinear_Act(layer)
    return model
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = QuantDynaLinear(config.hidden_size, self.all_head_size, config.num_attention_heads, dyna_dim=[False, True])
        self.key = QuantDynaLinear(config.hidden_size, self.all_head_size, config.num_attention_heads, dyna_dim=[False, True])
        self.value = QuantDynaLinear(config.hidden_size, self.all_head_size, config.num_attention_heads, dyna_dim=[False, True])
    ...
    def transpose_for_scores(self, x):
        ...
        #print("after", new_x_shape)
        x = x.view(*new_x_shape)
    
        return x.permute(0, 2, 1, 3)

    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        #print("mixed_query_layer before", hidden_states.shape)
        mixed_query_layer = self.query(hidden_states)
        #print("mixed_query_layer after", mixed_query_layer.shape)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        # dense layer for adaptive width
        self.dense = QuantDynaLinear(config.hidden_size, config.hidden_size, config.num_attention_heads, dyna_dim=[True, False])
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        # dense layer for adaptive width
        self.dense = QuantDynaLinear(config.hidden_size, config.intermediate_size,
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        # dense layer for adaptive width
        self.dense = QuantDynaLinear(config.intermediate_size, config.hidden_size,
