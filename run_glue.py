...
def main():
    ...
    default_params = {
        ...
        "mrpc": {"num_train_epochs": 10, "max_seq_length": 128, "learning_rate": 3e-5, "train_batch_size": 16},
        "rte": {"num_train_epochs": 10, "max_seq_length": 128, "learning_rate": 3e-5, "train_batch_size": 16, "weight_decay": 0.1},
    ...
    if task_name in default_params:    
        ...
        if "weight_decay" in default_params[task_name]:
            args.weight_decay = default_params[task_name]["weight_decay"]
        else:
            args.weight_decay = 0.01

        if "warmup_proportion" in default_params[task_name]:
            args.warmup_proportion = default_params[task_name]["warmup_proportion"]
        else:
            args.warmup_proportion = 0.06

        if "eps" in default_params[task_name]:
            args.eps = default_params[task_name]["eps"]
        else:
            args.eps = 1e-8
    ...
    # if not args.do_eval:
    #     teacher_model = BertForSentencePairClassification.from_pretrained(args.teacher_model, num_labels=num_labels)
    #     teacher_model.to(device)
    logger.info('New args: {}'.format(args))
